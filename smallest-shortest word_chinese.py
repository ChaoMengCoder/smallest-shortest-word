...
def parse_group_element(element_str):
    ...
    def parse_exponent(start_idx):
        ...
        if not exp_str or (len(exp_str) == 1 and exp_str[0] in '+-'):
            raise ValueError(f"无效的指数：{exp_str}")
    ...
    while i < n:
        ...
        if s[i] == '(':
            ...
            while j < n and depth > 0:
                ...
                j += 1
            if depth > 0:
                raise ValueError(f"括号不匹配：{original_s}")
            content = s[i + 1:j - 1]
            i = j - 1
            if j < n and s[j] == '^':
                j += 1
            ...
            if i + 2 < n and s[i:i + 3] in ['a^-', 'b^-']:
                ...
                raise ValueError(f"无法识别的字符：'{s[i]}'，位置 {i}，仅支持 a、b、A、B、括号()及幂次^n")
    return parsed
...
class FreeGroupWord:
    ...
    def _validate_cyclic_reduced(self):
        ...
        if temp[0] == INVERSE[temp[-1]]:
            raise ValueError(f"字 {''.join(temp)} 不是循环既约字！首尾字符 {temp[0]} 和 {temp[-1]} 互为逆元")
        for i in range(len(temp) - 1):
            if temp[i] == INVERSE[temp[i + 1]]:
                raise ValueError(
                    f"字 {''.join(temp)} 不是既约字！位置{i}和{i + 1}的字符 {temp[i]} 和 {temp[i + 1]} 互为逆元")
...
# ===================== 主程序入口 =====================
def main():
    print("===== 自由群 F (a,b) 自同构轨道中的最小最短字计算程序 =====")
    print("使用说明：")
    print("1. 输入格式：a、b、A=a^-1、B=b^-1")
    print("2. 输入 quit 退出程序")
    print("========================================\n")

    while True:
        # 获取用户输入
        user_input = input("请输入自由群中的字：").strip()
        ...
        try:
            ...
            # 输出结果
            #print(f"\n输入字：{user_input}")
            print(f"该字所在的自同构群轨道中的最小最短字：{oriented_repr}")
            print("========================================\n")
